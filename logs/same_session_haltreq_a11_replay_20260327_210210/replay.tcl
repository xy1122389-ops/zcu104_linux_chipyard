set outdir [file normalize "/root/chipyard/fpga/logs/same_session_haltreq_a11_replay_20260327_210210"]
...
for {set idx 1} {$idx <= $repeats} {incr idx} {
  ...
  puts $fp_xsdb {if {$preclear} {
set seq [jtag sequence]
$seq irshift -integer 12 0x926
}}
  puts $fp_xsdb {if {$preclear} {set _dummy 0}}
  puts $fp_xsdb [format {if {$preclear} {$seq drshift -capture -integer %d 0x%x}} 53 0x410000000653]
  puts $fp_xsdb {if {$preclear} {puts "WRITE_RESUMEREQ=[$seq run -hex]"}}
  puts $fp_xsdb {if {$preclear} {
set seq [jtag sequence]
$seq irshift -integer 12 0x926
}}
  puts $fp_xsdb [format {if {$preclear} {$seq drshift -capture -integer %d 0x%x}} 53 0x53]
  puts $fp_xsdb {if {$preclear} {puts "PRECLEAR_NOP1=[$seq run -hex]"}}
  puts $fp_xsdb {if {$preclear} {
set seq [jtag sequence]
$seq irshift -integer 12 0x926
}}
  puts $fp_xsdb [format {if {$preclear} {$seq drshift -capture -integer %d 0x%x}} 53 0x53]
  puts $fp_xsdb {if {$preclear} {puts "PRECLEAR_NOP2=[$seq run -hex]"}}
  puts $fp_xsdb {if {$preclear} {
set seq [jtag sequence]
$seq irshift -integer 12 0x926
}}
  puts $fp_xsdb [format {if {$preclear} {$seq drshift -capture -integer %d 0x%x}} 53 0x400000000653]
  puts $fp_xsdb {if {$preclear} {puts "WRITE_DMACTIVE=[$seq run -hex]"}}
  puts $fp_xsdb {if {$preclear} {
set seq [jtag sequence]
$seq irshift -integer 12 0x926
}}
  puts $fp_xsdb [format {if {$preclear} {$seq drshift -capture -integer %d 0x%x}} 53 0x53]
  puts $fp_xsdb {if {$preclear} {puts "PRECLEAR_NOP3=[$seq run -hex]"}}
  puts $fp_xsdb {if {$preclear} {
set seq [jtag sequence]
$seq irshift -integer 12 0x926
}}
  puts $fp_xsdb [format {if {$preclear} {$seq drshift -capture -integer %d 0x%x}} 53 0x53]
  puts $fp_xsdb {if {$preclear} {puts "PRECLEAR_NOP4=[$seq run -hex]"}}
  puts $fp_xsdb {set seq [jtag sequence]}
  puts $fp_xsdb {$seq irshift -integer 12 0x926}
