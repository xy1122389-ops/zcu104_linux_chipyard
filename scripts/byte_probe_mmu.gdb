# byte_probe_mmu.gdb — Load and run the bare-metal MMU byte-access probe
#
# Usage:
#   JLINK_PORT=12331 riscv64-unknown-linux-gnu-gdb -batch -x scripts/byte_probe_mmu.gdb
#
# Prerequisites:
#   - tests/byte_probe_mmu.bin built and present
#   - J-Link GDB server running (via relay or direct)
...
# ---- Read results via SBA (dump binary, then parse) ----
res_file = "/tmp/byte_probe_result.bin"
...
gdb.write("\n" + "="*60 + "\n")
gdb.write("  BYTE PROBE MMU — RESULTS\n")
gdb.write("="*60 + "\n")
...
if magic_str == "UNKNOWN(0x0)":
    gdb.write("\n  *** Results area is all zeros — test may not have run! ***\n")
    gdb.write("  Check: did the CPU start from 0x81200000?\n")
    gdb.write(f"  Current PC: 0x{pc:016x}\n")
    # Try reading a few memory words for debugging
    for off in [0, 8, 16]:
        try:
            v = u64(off)
            gdb.write(f"  result[0x{off:02x}] = 0x{v:016x}\n")
        except:
            pass
...
# Save summary
summary = f"/tmp/byte_probe_mmu_result.txt"
