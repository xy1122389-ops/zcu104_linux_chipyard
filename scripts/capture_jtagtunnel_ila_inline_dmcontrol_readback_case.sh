...
if [[ $# -lt 3 ]]; then
  echo "Usage: $0 <ila_dir> <label> <dmcontrol_case:dmactive|haltreq|resumereq> [jtag_hz]" >&2
  exit 2
fi
...
mkdir -p "$OUTDIR"
XSDB_SCRIPT="$OUTDIR/xsdb_dmcontrol_readback_case.tcl"
...
read -r rd10_bits rd10_val < <(pack_dmi_read_addr 0x10)
read -r rd11_bits rd11_val < <(pack_dmi_read_addr 0x11)
...
puts "NOP1_$CASE_NAME=[\$seq run -hex]"
set seq [jtag sequence]
\$seq irshift -integer 12 0x926
\$seq drshift -capture -integer $nop_bits $nop_val
puts "NOP2_$CASE_NAME=[\$seq run -hex]"
set seq [jtag sequence]
\$seq irshift -integer 12 0x926
\$seq drshift -capture -integer $rd10_bits $rd10_val
puts "READ_DMCONTROL_$CASE_NAME=[\$seq run -hex]"
set seq [jtag sequence]
\$seq irshift -integer 12 0x926
\$seq drshift -capture -integer $nop_bits $nop_val
puts "NOPR10_1_$CASE_NAME=[\$seq run -hex]"
set seq [jtag sequence]
\$seq irshift -integer 12 0x926
\$seq drshift -capture -integer $nop_bits $nop_val
puts "NOPR10_2_$CASE_NAME=[\$seq run -hex]"
set seq [jtag sequence]
\$seq irshift -integer 12 0x926
\$seq drshift -capture -integer $rd11_bits $rd11_val
puts "READ_DMSTATUS_$CASE_NAME=[\$seq run -hex]"
set seq [jtag sequence]
\$seq irshift -integer 12 0x926
\$seq drshift -capture -integer $nop_bits $nop_val
puts "NOPR11_1_$CASE_NAME=[\$seq run -hex]"
set seq [jtag sequence]
\$seq irshift -integer 12 0x926
\$seq drshift -capture -integer $nop_bits $nop_val
puts "NOPR11_2_$CASE_NAME=[\$seq run -hex]"
exit
EOF
