...
if [[ $# -lt 4 ]]; then
  echo "Usage: $0 <probes_ltx> <label> <dr_bits> <dr_hex> [jtag_hz]" >&2
  exit 2
fi
...
OUTDIR="$FPGA_DIR/logs/jtagtunnel_ila_xsdb_reuse_${LABEL}_${STAMP}"
mkdir -p "$OUTDIR"
...
/root/.local/bin/vivado -mode batch -source "$SCRIPT_DIR/arm_jtagtunnel_ila_existing_zcu104.tcl" -tclargs \
  -probes_path "$LTXFILE" \
