...
mkdir -p "$OUTDIR"
XSDB_SCRIPT="$OUTDIR/xsdb_dmcontrol_only_case.tcl"
...
read -r rd_bits rd_val < <(pack_dmi_read_dmcontrol)
...
\$seq irshift -integer 12 0x926
\$seq drshift -capture -integer $nop_bits $nop_val
puts "NOPR1_$CASE_NAME=[\$seq run -hex]"
set seq [jtag sequence]
\$seq irshift -integer 12 0x926
\$seq drshift -capture -integer $nop_bits $nop_val
puts "NOPR2_$CASE_NAME=[\$seq run -hex]"
exit
EOF
