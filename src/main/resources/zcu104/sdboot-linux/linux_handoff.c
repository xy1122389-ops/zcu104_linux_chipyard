// CPU-driven validation payload for ZCU104.
#include <stdint.h>
...
#define PRE_DDR_HEARTBEATS 120u
...
static void pre_ddr_heartbeat(unsigned count)
{
  unsigned i;

  for (i = 0; i < count; ++i) {
    ds38_toggle();
    uart_puts("pre-ddr rocket_alive heartbeat=");
    print_dec(i);
    uart_puts(" ds38=");
...
int main(void)
{
  ...
  uart_puts("pre-ddr hold start\n");
  pre_ddr_heartbeat(PRE_DDR_HEARTBEATS);
  uart_puts("pre-ddr hold done\n");
  uart_puts("entering ddr stage\n");
