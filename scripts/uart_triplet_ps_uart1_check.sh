...
OUTDIR="logs/uart_ps_uart1_triplet_${TS}"
mkdir -p "$OUTDIR"
...
for port in "${PORTS[@]}"; do
  PWSH_SCRIPT="/tmp/uart_ps1_${TS}_${port}.ps1"
...
PS1
  ...
  powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$WIN_SCRIPT" -PortName "$port" -OutDir "$WIN_OUTDIR" -DurationSec "$DUR" -BaudRate "$BAUD" >/tmp/uart_ps1_${TS}_${port}.out 2>&1 &
  CAP_PIDS+=("$!")
done

sleep 2
bash "$SCRIPT_DIR/xsdb_ps_uart1_inject_test.sh" > "$OUTDIR/inject.log" 2>&1 || true
...
for port in "${PORTS[@]}"; do
  grep -a -n 'PS_UART1_TEST_115200' "$OUTDIR/${port}.log" > "$OUTDIR/${port}.keywords.log" || true
done
