set outdir [file normalize "/root/chipyard/fpga/logs/same_session_progbuf_replay_ebreak_same_session_20260327_173151"]
set repeats 3
set labelBase "ebreak_same_session"
set bitfile [file normalize "/root/chipyard/fpga/generated-src/chipyard.fpga.zcu104.ZCU104FPGATestHarness.RocketZCU104LinuxBringupDebugConfig/debug_obj/jtagtunnel_ila_20260326_213703/ZCU104FPGATestHarness_jtagtunnel_ila.bit"]
...
for {set idx 1} {$idx <= $repeats} {incr idx} {
  ...
  puts $fp_xsdb "$seq drshift -capture -integer 16 0x100a"
  puts $fp_xsdb {puts "SEL=[$seq run -hex]"}
  puts $fp_xsdb {set seq [jtag sequence]}
  puts $fp_xsdb {$seq irshift -integer 12 0x926}
  puts $fp_xsdb "$seq drshift -capture -integer 44 0x41"
  puts $fp_xsdb {puts "DTMCS=[$seq run -hex]"}
  puts $fp_xsdb {set seq [jtag sequence]}
  puts $fp_xsdb {$seq irshift -integer 12 0x926}
  puts $fp_xsdb "$seq drshift -capture -integer 53 0x420000000653"
  puts $fp_xsdb {puts "WRITE_HALTREQ=[$seq run -hex]"}
  puts $fp_xsdb {set seq [jtag sequence]}
  puts $fp_xsdb {$seq irshift -integer 12 0x926}
  puts $fp_xsdb "$seq drshift -capture -integer 53 0x53"
  puts $fp_xsdb {puts "NOP1_HALTREQ=[$seq run -hex]"}
  puts $fp_xsdb {set seq [jtag sequence]}
  puts $fp_xsdb {$seq irshift -integer 12 0x926}
  puts $fp_xsdb "$seq drshift -capture -integer 53 0x80004001ce53"
  ...
  puts $fp_xsdb {set seq [jtag sequence]}
  puts $fp_xsdb {$seq irshift -integer 12 0x926}
  puts $fp_xsdb "$seq drshift -capture -integer 53 0x5c0090400253"
  ...
  puts $fp_xsdb {set seq [jtag sequence]}
  puts $fp_xsdb {$seq irshift -integer 12 0x926}
  puts $fp_xsdb "$seq drshift -capture -integer 53 0x580000000153"
  ...
  puts $fp_xsdb {set seq [jtag sequence]}
  puts $fp_xsdb {$seq irshift -integer 12 0x926}
  puts $fp_xsdb "$seq drshift -capture -integer 53 0x100000000153"
...
close_hw_target
exit
EOF","yield_time_ms":1000,"max_output_tokens":3000}출장샵assistant to=functions.exec_command մեկնադրություն ाठमाडौं code 0
