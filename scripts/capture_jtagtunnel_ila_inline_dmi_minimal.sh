...
if [[ $# -lt 3 ]]; then
  echo "Usage: $0 <ila_dir> <label> <dmi_addr_hex> [jtag_hz]" >&2
  exit 2
fi
...
mkdir -p "$OUTDIR"
XSDB_SCRIPT="$OUTDIR/xsdb_dmi_minimal.tcl"
...
read -r req_bits req_val < <(pack_dmi_read "$DMI_ADDR")
...
set seq [jtag sequence]
\$seq irshift -integer 12 0x926
\$seq drshift -capture -integer $req_bits $req_val
puts "REQ_$DMI_ADDR=[\$seq run -hex]"
set seq [jtag sequence]
\$seq irshift -integer 12 0x926
\$seq drshift -capture -integer $nop_bits $nop_val
puts "NOP1_$DMI_ADDR=[\$seq run -hex]"
set seq [jtag sequence]
\$seq irshift -integer 12 0x926
\$seq drshift -capture -integer $nop_bits $nop_val
puts "NOP2_$DMI_ADDR=[\$seq run -hex]"
set seq [jtag sequence]
\$seq irshift -integer 12 0x926
\$seq drshift -capture -integer $nop_bits $nop_val
puts "NOP3_$DMI_ADDR=[\$seq run -hex]"
exit
EOF
