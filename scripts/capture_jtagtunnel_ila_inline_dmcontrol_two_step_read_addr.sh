...
if [[ $# -lt 4 ]]; then
  echo "Usage: $0 <ila_dir> <label> <sequence:haltreq_then_resumereq|haltreq_then_dmactive> <read_addr_hex> [jtag_hz]" >&2
  exit 2
fi
...
mkdir -p "$OUTDIR"
XSDB_SCRIPT="$OUTDIR/xsdb_dmcontrol_two_step_read_addr.tcl"
...
puts "READ_${READ_ADDR}_$SEQ_NAME=[\$seq run -hex]"
set seq [jtag sequence]
\$seq irshift -integer 12 0x926
\$seq drshift -capture -integer $nop_bits $nop_val
puts "NOPR1_${READ_ADDR}_$SEQ_NAME=[\$seq run -hex]"
set seq [jtag sequence]
\$seq irshift -integer 12 0x926
\$seq drshift -capture -integer $nop_bits $nop_val
puts "NOPR2_${READ_ADDR}_$SEQ_NAME=[\$seq run -hex]"
exit
EOF
