...
ILA_DIR="${1:?usage: $0 <ila_dir> <tx|rx|enq> <outdir> [trigger_value] }"
TRIGGER_PROBE="${2:?usage: $0 <ila_dir> <tx|rx|enq> <outdir> [trigger_value] }"
OUTDIR="${3:?usage: $0 <ila_dir> <tx|rx|enq> <outdir> [trigger_value] }"
TRIGGER_VALUE="${4:-0}"

mkdir -p "$OUTDIR"
BITFILE="$ILA_DIR/ZCU104FPGATestHarness_uart_ila.bit"
LTXFILE="$ILA_DIR/uart_ila.ltx"
ARM_STATE="$OUTDIR/arm_state.txt"
CSV_OUT="$OUTDIR/uart_ila.csv"
UPLOAD_STATE="$OUTDIR/upload_state.txt"

/root/.local/bin/vivado -mode batch -source "$SCRIPT_DIR/arm_uart_ila_zcu104.tcl" -tclargs \
