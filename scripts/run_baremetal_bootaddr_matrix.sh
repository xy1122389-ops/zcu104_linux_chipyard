#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
FW=${1:-/root/chipyard/fpga/src/main/resources/zcu104/sdboot/build/sdboot.bin}
shift || true

BOOTADDRS=("$@")
if [[ ${#BOOTADDRS[@]} -eq 0 ]]; then
  BOOTADDRS=(0x80000000 0x80002000 0x80010000)
fi

TS=$(date +%Y%m%d_%H%M%S)
OUTDIR="/root/chipyard/fpga/logs/baremetal_bootaddr_matrix_${TS}"
mkdir -p "$OUTDIR"

for addr in "${BOOTADDRS[@]}"; do
  tag=$(echo "$addr" | tr '[:lower:]' '[:upper:]' | tr -cd '0-9A-FX')
  log="$OUTDIR/${tag}.log"
  {
    echo "BOOTADDR=$addr"
    echo "FW=$FW"
    echo "==== psu_init + bitstream ===="
    bash "$SCRIPT_DIR/run_ps_ddr_init_linux.sh"
    echo
    echo "==== load baremetal bin ===="
    bash "$SCRIPT_DIR/load_linux_fw_payload.sh" "$FW" "$addr"
    echo
    echo "==== pulse tile reset and kick ===="
    bash "$SCRIPT_DIR/pulse_tile_reset_and_kick.sh"
    echo
    echo "==== gpio toggle probe ===="
    bash "$SCRIPT_DIR/xsdb_gpio_toggle_probe.sh"
  } | tee "$log"
done

echo "OUTDIR=$OUTDIR"
