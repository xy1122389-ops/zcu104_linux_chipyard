set outdir [file normalize "/root/chipyard/fpga/logs/same_session_haltreq_a11_replay_20260327_213548"]
...
for {set idx 1} {$idx <= $repeats} {incr idx} {
  ...
  puts $fp_xsdb {set seq [jtag sequence]}
  puts $fp_xsdb {$seq irshift -integer 12 0x926}
  puts $fp_xsdb [format {$seq drshift -capture -integer %d 0x%x} 53 0x410000000653]
  ...
  puts $fp_xsdb {set seq [jtag sequence]}
  puts $fp_xsdb {$seq irshift -integer 12 0x926}
  puts $fp_xsdb [format {$seq drshift -capture -integer %d 0x%x} 53 0x400000000653]
  ...
  puts $fp_xsdb {set seq [jtag sequence]}
  puts $fp_xsdb {$seq irshift -integer 12 0x926}
  puts $fp_xsdb [format {$seq drshift -capture -integer %d 0x%x} 53 0x400000000153]
  puts $fp_xsdb {puts "READ_0x10=[$seq run -hex]"}
  puts $fp_xsdb {set seq [jtag sequence]}
  puts $fp_xsdb {$seq irshift -integer 12 0x926}
  puts $fp_xsdb [format {$seq drshift -capture -integer %d 0x%x} 53 0x53]
