set outdir [file normalize "/root/chipyard/fpga/logs/same_session_ebreak_replay_20260327_204627"]
...
for {set idx 1} {$idx <= $repeats} {incr idx} {
  ...
  puts $fp_xsdb {set seq [jtag sequence]}
  puts $fp_xsdb {$seq irshift -integer 12 0x926}
  puts $fp_xsdb [format {$seq drshift -capture -integer %d 0x%x} 53 0x80004001ce53]
  ...
  puts $fp_xsdb {set seq [jtag sequence]}
  puts $fp_xsdb {$seq irshift -integer 12 0x926}
  puts $fp_xsdb [format {$seq drshift -capture -integer %d 0x%x} 53 0x5c0090400253]
  ...
  puts $fp_xsdb {set seq [jtag sequence]}
  puts $fp_xsdb {$seq irshift -integer 12 0x926}
  puts $fp_xsdb [format {$seq drshift -capture -integer %d 0x%x} 53 0x580000000153]
  ...
  puts $fp_xsdb {set seq [jtag sequence]}
  puts $fp_xsdb {$seq irshift -integer 12 0x926}
  puts $fp_xsdb [format {$seq drshift -capture -integer %d 0x%x} 53 0x100000000153]
